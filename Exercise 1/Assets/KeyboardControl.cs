...
public class KeyboardControl : MonoBehaviour
{
    ...
    void Run()
    {
        ...
        if (Input.GetAxisRaw("Horizontal") < 0)
        {
            ...
            rb.AddForce(new Vector2(-0.1f, 0), ForceMode.Impulse);
        }
        if (Input.GetAxisRaw("Horizontal") > 0)
        {
            rb.AddForce(new Vector2(0.1f, 0), ForceMode.Impulse);
        ...
        if (Input.GetKey("up"))
        {
            rb.AddForce(new Vector2(0, 0.1f), ForceMode.Impulse);
        }
    ...
    void Attack()
    {
        if (Input.GetButtonDown("Fire1"))
        {
            ...
            ball.GetComponent<Rigidbody>().AddRelativeForce(new Vector3 //Rigidbody2D
                (0, 1500, 0));
